...
// Access the API key from environment secrets
// Note: You must set this secret via CLI: firebase functions:secrets:set OPENAI_API_KEY
 ...
 */
exports.analyzeClothingItem = functions.runWith({ secrets: ["OPENAI_API_KEY"], timeoutSeconds: 60, memory: "1GB" })
    .https.onCall(async (data, context) => {
        ...
        if (!base64Image) {
            throw new functions.https.HttpsError('invalid-argument', 'The function must be called with a "base64Image" argument.');
        }
        ...
        if (results.length === 0) {
            throw new functions.https.HttpsError('internal', 'Analysis failed to return valid results.');
        }
 ...
 */
exports.generateOutfitSuggestions = functions.runWith({ secrets: ["OPENAI_API_KEY"], timeoutSeconds: 60 })
    .https.onCall(async (data, context) => {
        if (!context.auth) {
            throw new functions.https.HttpsError('unauthenticated', 'The function must be called while authenticated.');
        }

        const { availableItems, criteria } = data;
